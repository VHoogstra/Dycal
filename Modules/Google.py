...
class Google:
  ...
  def updateEventData(self, gevent, dyflexysEvent: EventDataShift):
    Logger.getLogger(__name__).info('update new Calendar event from ' + dyflexysEvent.date)
    ...
    if 'description' in gevent and gevent['description'] != None:
      ...
      Logger.getLogger(__name__).info('new description')
      startAutoGenerated = gevent['description'].find(Constants.DESCRIPTION_PREFIX)
      Logger.getLogger(__name__).info(startAutoGenerated)
  ...
  def compare_google_event_to_dyflexis(self, dyflexis: EventDataShift, gevent):
    """
    check if there is a difference in google vs dyflexis
    :param dyflexis:
    :param gevent:
    :return:

    compare op omschrijving, naam, datum, tijd
    """
    if arrow.get(dyflexis.start_date, tzinfo=Constants.timeZone) != arrow.get(gevent['start']['dateTime'],
                                                                              tzinfo=Constants.timeZone):
      return False
    if arrow.get(dyflexis.end_date, tzinfo=Constants.timeZone) != arrow.get(gevent['end']['dateTime'],
                                                                            tzinfo=Constants.timeZone):
      return False

    # check het auto gen deel van de omschrijving, rest maakt ons niet uit
    startAutoGenerated = gevent['description'].find(Constants.DESCRIPTION_PREFIX)
    desc = gevent['description'][startAutoGenerated:]
    desb = dyflexis.description + "\n" + dyflexis.id
    if desb[0:1] == "\n":
      desb = desb[1:]
    if desb != desc:
      return False
    if dyflexis.title != gevent['summary']:
      return False
    return True

  def parseEventsToGoogle(self, googleCal: list, dyflexisEvents: list[EventDataShift], periods=None):
    ...
    for shift in dyflexisEvents:
      ...
      # if no event is found in google, create one
  ...
  def sortRemoval(self, returnObject: ExportReturnObject, google_events, periods):
    ...
    for gEvent in google_events:
      ...
      if 'start' in gEvent and 'dateTime' in gEvent['start']:
        ...
        # als vandaag eerder dan end is, doen we geen remove
        if today < end:
          for period in periods:
            period_month = arrow.get(period, tzinfo=Constants.timeZone).month
            if (start == period_month or end.month == period_month):
              periodBreak = False
      if periodBreak:
        continue
      ...
      if 'description' not in gEvent:
        continue
      if gEvent['description'] != None and Dyflexis.DESCRIPTION_PREFIX not in gEvent['description']:
        continue
